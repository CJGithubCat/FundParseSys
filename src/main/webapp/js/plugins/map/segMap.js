...
var SEGMap = function(divId, mapType){
	...
	this.centerAndZoom_str = function(str, level){
		_proxyMap.centerAndZoom_str(str, level);
	};
	...
	this.getBoundary=function(name, id, strokeColor, strokeWeight, strokeOpacity, fillColor, fillOpacity){
		return _proxyMap.getBoundary(name, id, strokeColor, strokeWeight, strokeOpacity, fillColor, fillOpacity);
	};
	//添加画图工具
	this.getDrawingManager=function(strokeColor,fillColor,strokeWeight,strokeOpacity,fillOpacity,strokeStyle,isShow,point){
		return _proxyMap.newDrawingManager(strokeColor,fillColor,strokeWeight,strokeOpacity,fillOpacity,strokeStyle,isShow,point);
	}
	//添加单点图像
	this.getMarker=function(item,icon,imgWeight,imgHight){
		return _proxyMap.newMarker(item,icon,imgWeight,imgHight);
	}
	
	//清除覆盖物
	this.clearOverlays=function(remp){
		_proxyMap.clearOverlays(remp);
	}
	//创建折线
	//返回SEGPolyline的实例
	...
	//拉框放大地图
	this.enlargeRectangleZoom = function(){
		_proxyMap.enlargeRZ();
	};
	//拉框缩小地图
	this.reduceRectangleZoom = function(){
		_proxyMap.reduceRZ();
	}
	//开启拖拽功能
	this.enableDragging = function(){
		_proxyMap.enableDrag();
	}
	//历史回放
...
//绘图
var SEGDrawingManager = function(){
	//this.markerType = 5;
	this.target = null;
	this.strokeColor = null;
	this.strokeWeight = null;
