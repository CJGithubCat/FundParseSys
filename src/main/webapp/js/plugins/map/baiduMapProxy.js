var BaiduMapProxy = function(divId, segMap){
	...
	var _myDragEnlarge = new BMapLib.RectangleZoom(_map, {
		followText: "拖拽鼠标放大地图,点击菜单栏手型按钮结束",
		zoomType:0
	});
	var _myDragReduce = new BMapLib.RectangleZoom(_map, {
		followText: "拖拽鼠标缩小地图,点击菜单栏手型按钮结束",
		zoomType:1
	});
	...
	//鼠标拉框放大地图
	this.enlargeRZ = function(){
		reduceRZClose();
		_myDragEnlarge.open();
	};
	
	//关闭鼠标拉框放大地图
	function enlargeRZClose(){
		_myDragEnlarge.close();
	};
	//鼠标拉框缩小地图
	this.reduceRZ = function(){
		enlargeRZClose();
		_myDragReduce.open();
	};
	
	//关闭鼠标拉框缩小地图
	function reduceRZClose(){
		_myDragReduce.close();
	}
	//开启拖拽
	this.enableDrag = function(){
		...
		reduceRZClose();
		_map.enableDragging();
		_map.setDefaultCursor("url('bird.cur')");
	}
	...
	this.centerAndZoom_str = function(str, level){
		_map.centerAndZoom(str, level);
	};
	...
	this.newCircle = function(lon, lat, radius, id, strokeColor, strokeWeight, strokeOpacity, fillColor, fillOpacity){
		...
		_map.addOverlay(circle);//添加覆盖物到地图上
	...
	this.newRectangle = function(lon1, lat1, lon2, lat2, id, strokeColor, strokeWeight, strokeOpacity, fillColor, fillOpacity){
		...
		_map.addOverlay(polygon);//添加覆盖物到地图上
	...
	this.newPolygon = function(ps, id, strokeColor, strokeWeight, strokeOpacity, fillColor, fillOpacity){
		...
		_map.addOverlay(polygon);//添加覆盖物到地图上
		//_map.setViewport(polygon.getPath());
	...
	//周伟===省市行政图形
	this.getBoundary= function(name, id, strokeColor, strokeWeight, strokeOpacity, fillColor, fillOpacity){
		var areaName=name;
		var sw = typeof(strokeWeight)=="undefined"? 2: strokeWeight;
		var so = typeof(strokeOpacity)=="undefined"? 1: strokeOpacity;
		var fo = typeof(fillOpacity)=="undefined"? 0.65: fillOpacity;
		...
		bdary.get(areaName, function(rs){       //获取行政区域
			...
			for(var i = 0; i < count; i++){
				...
				_map.setViewport(ply.getPath());    //调整视野         
			}                
		});   
		var sEGBoundary = new SEGBoundary();
		sEGBoundary.target = ply;
		sEGBoundary.id = id;
		sEGBoundary.name=areaName;
		sEGBoundary.strokeColor = strokeColor;
		sEGBoundary.strokeWeight = strokeWeight;
		sEGBoundary.strokeOpacity = strokeOpacity;
		sEGBoundary.fillColor = fillColor;
		sEGBoundary.fillOpacity = fillOpacity;
		sEGBoundary.zoom = _map.getZoom();
		return sEGBoundary;
	};
	//画图工具加载
	this.newDrawingManager=function(strokeColor,fillColor,strokeWeight,strokeOpacity,fillOpacity,strokeStyle,isShow,point){
		var styleOptions = {
		        strokeColor:strokeColor,//"red",    //边线颜色。
		        fillColor:fillColor,//"red",      //填充颜色。当参数为空时，圆形将没有填充效果。
		        strokeWeight: strokeWeight,//3,       //边线的宽度，以像素为单位。
		        strokeOpacity: strokeOpacity,//0.8,	   //边线透明度，取值范围0 - 1。
		        fillOpacity: fillOpacity,//0.6,      //填充的透明度，取值范围0 - 1。
		        strokeStyle: strokeStyle//'solid' //边线的样式，solid或dashed。
		    }
		var mods;
		if(point=="index"){
			mods=[BMAP_DRAWING_CIRCLE];
		}else{
			mods=[BMAP_DRAWING_CIRCLE,BMAP_DRAWING_POLYGON,
	...
	this.newMarker=function(item,icon,imgWeight,imgHight){
		if(item.lon==0&&item.lat==0){
			alert("车牌为:"+item.plateNo+"车辆暂无上报记录,地图上无坐标!");
		}else{
			var pt = new BMap.Point(item.lon, item.lat);
			var myIcon = new BMap.Icon(icon, new BMap.Size(imgWeight,imgHight));
			var marker = new BMap.Marker(pt,{icon:myIcon});  // 创建标注
			_map.addOverlay(marker); 
			var opts = {
					  width : 200,     // 信息窗口宽度
					  height: 500,     // 信息窗口高度
					  //title : "实时车辆详细信息"  // 信息窗口标题
			...
			var str="车牌号："+item.plateNo+"<br>"
					 +"车载号码："+item.callLetter+"<br>"
					 +"所属机构："+item.subcoName+"<br>"
					 +"驾驶员："+item.driverName+"<br>"
					 +sex
					 +"工号："+item.workNo+"<br>"
					 +"联系方式："+item.phone+"<br>"
					 +"身份证号："+item.idNo+"<br>"
					 
					 +"定位时间："+item.gpsTime+"<br>"
					 +"经度："+item.lon+"<br>"
					 +"纬度："+item.lat+"<br>"
					 +"实时速度："+item.speed+"公里/小时<br>"
					 +"方向："+item.course+"<br>"
					 +"总里程："+item.totalMile+"m<br>"
					 +"油量："+item.oil+"L<br>"
					
					 +"服务证号："+item.scNo+"<br>"
					 +"服务证有效时间至："+item.scEdate+"<br>"
					 +"登记证号："+item.regNo+"<br>"
					 +"登记证有效时间至："+item.regEdate+"<br>"
					 +"驾驶证号："+item.drNo+"<br>"
					 +"准驾车型："+item.drClass+"<br>"
					 +"驾驶证有效期至："+item.drEdate+"<br>";
			...
			var  sEGSimpleMarker =new SEGSimpleMarker();
			sEGSimpleMarker.target = marker;
			sEGSimpleMarker.lon = item.lon;
			sEGSimpleMarker.lat = item.lat;
			return sEGSimpleMarker;
		}
	...
	this.clearOverlays=function(remp){
		_map.clearOverlays();
	};
    		...
    		switch (curTool) {
				...
				case "rect":
					...
					if(curToolCallBack && rectangle_marker){
						...
						segRectangle.zoom=_map.getZoom();
