var QQMapProxy = function(divId, segMap){
	...
	this.newCircle = function(lon, lat, radius, id, strokeColor, strokeWeight, strokeOpacity, fillColor, fillOpacity){
		...
		var opts = {
			...
			clickable: false,
			visible : true,
			fillColor: qq.maps.Color.fromHex(fillColor || "white", fo),
			strokeColor: qq.maps.Color.fromHex(strokeColor || "blue", so),
			strokeWeight: sw
		}; 
			if(circle!=null){
				...
				this.clearOverlays(dpolygon);
			}
			circle= new qq.maps.Circle(opts);
			//腾讯添加圆形
		    circle.setMap(_map);
	...
	this.newRectangle = function(lon1, lat1, lon2, lat2, id, strokeColor, strokeWeight, strokeOpacity, fillColor, fillOpacity){
		...
		polygon = new qq.maps.Polygon(opts);
		//腾讯添加圆形
		polygon.setMap(_map);
	...
	this.newPolygon = function(ps, id, strokeColor, strokeWeight, strokeOpacity, fillColor, fillOpacity){
		...
		dpolygon = new qq.maps.Polygon(opts);
		//腾讯添加圆形
		dpolygon.setMap(_map);
		
		var segPolygon = new SEGPolygon();
		segPolygon.target = dpolygon;
		segPolygon.ps = ps;
		segPolygon.id = id;
	...
	this.newDrawingManager=function(strokeColor,fillColor,strokeWeight,strokeOpacity,fillOpacity,strokeStyle,isShow,point){
		...
		var sw = typeof(strokeWeight)=="undefined"? 2: strokeWeight;
		if(point=="index"){
			mods=[qq.maps.drawing.OverlayType.RECTANGLE];
		}else{
			mods=[
		...
		if(drawingManager!=null){
		  ...
		  drawingManager = new qq.maps.drawing.DrawingManager({
              ...
              circleOptions: {
                  fillColor: new qq.maps.Color.fromHex(fillColor || "white", fo),
                  strokeColor: new qq.maps.Color.fromHex(strokeColor || "blue", fo),
                  strokeWeight: sw,
                  clickable: false
              }
          });
		  	drawingManager.setMap(_map);
			var sEGDrawingManager = new SEGDrawingManager();
				sEGDrawingManager.target = drawingManager;
				sEGDrawingManager.strokeColor = strokeColor;
				sEGDrawingManager.strokeWeight = strokeWeight;
				sEGDrawingManager.strokeOpacity = strokeOpacity;
				sEGDrawingManager.fillColor = fillColor;
				sEGDrawingManager.fillOpacity = fillOpacity;
				
		  return sEGDrawingManager;
	}
	
	var  markersArray = [];
	this.newMarker=function(lon, lat,icon,imgWeight,imgHight){
		 var center=new qq.maps.LatLng(lat,lon);
		// var anchor = new qq.maps.Point(0, 0),
          size = new qq.maps.Size(imgWeight, imgHight),
         // origin = new qq.maps.Point(0, 0),
          ic = new qq.maps.MarkerImage(
					           icon, 
					           size
					         //  origin
					          // anchor
		...
		var  sEGSimpleMarker =new SEGSimpleMarker();
			sEGSimpleMarker.target = markersArray;
			sEGSimpleMarker.lon = lon;
			sEGSimpleMarker.lat = lat;
	...
	this.clearOverlays=function(remp){
		if(remp.length==undefined){
			remp.setMap(null);
		}else{
			for(var i=0 ;i<remp.length;i++){
				remp[i].setMap(null);
			}
