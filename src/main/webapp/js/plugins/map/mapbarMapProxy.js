...
var MapbarMapProxy = function(divId, segMap){
	...
	this.newCircle = function(lon, lat, radius, id, strokeColor, strokeWeight, strokeOpacity, fillColor, fillOpacity){
		...
		circle = new MEllipse(point, radius, null, brush);
		_map.addOverlay(circle);
		this.centerAndZoom(lon,lat,10);
	...
	this.newRectangle = function(lon1, lat1, lon2, lat2, id, strokeColor, strokeWeight, strokeOpacity, fillColor, fillOpacity){
	    ...
	    rectangle = new MPolyline(path, brush);
	...
	this.newPolygon = function(ps, id, strokeColor, strokeWeight, strokeOpacity, fillColor, fillOpacity){
		...
		polygon = new MPolyline(path, brush);
...
//	}
	...
	this.newMarker=function(lon, lat,icon,imgWeight,imgHight){
	    var marker = new MMarker(  
	        new MPoint(lon,lat),  
	        new MIcon(icon,imgWeight,imgHight)  
	    ); 
	...
	this.clearOverlays=function(remp){
		if(remp.length==undefined){
			_map.removeOverlay(remp,true);
		}else{
			for(var i=0 ;i<remp.length;i++){
				_map.removeOverlay(remp[i],true);
			}
		}
